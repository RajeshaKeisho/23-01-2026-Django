...
def set_cookie(request):
    ...
    response.set_cookie("username", "Nitya", max_age=60)
    return response
...
def set_theme(request):
    if request.method == "POST":
        ...
        messages.success(request, f'The theme has be set to {theme}.')
        return response
...
def protected_view(request):
    if 'username' not in request.session:
        messages.error(request, 'You must be logged in first to access this data!')
        return redirect('login')
    return HttpResponse("This is a protected page. Only loged in users can see this.")